...
def create_queues_for_today():
    if frappe.flags.in_test:
        clinics = frappe.get_all(
            "Clinic",
            filters={"is_published": True, "name": "Test Clinic"},
            pluck="name",
        )
        # breakpoint()
    else:
    ...
    for clinic in clinics:
        # create an appointmen queue if doesn't exists, use ignore_if_duplicate=True
        # to ignore duplicate entry error
        shifts = frappe.get_all(
            "Schedule Shift", filters={"clinic": clinic}, pluck="name"
        )
        for shift in shifts:
            {
                frappe.get_doc(
